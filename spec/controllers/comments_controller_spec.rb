...
RSpec.describe Api::V1::CommentsController, type: :controller do
  ...
  describe 'GET index' do
    context 'with valid task_id' do
      let(:params) { { task_id: task.id, project_id: project.id } }
    ...
    context 'with invalid task_id' do
      let(:params) { { task_id: 0, project_id: project.id } }
  ...
  describe 'POST create' do
    ...
    context 'with valid body' do
      let(:params) { { task_id: task.id, project_id: project.id, body: FFaker::Lorem.paragraph } }
    ...
    context 'with invalid body' do
      let(:params) { { task_id: task.id, project_id: project.id, body: nil } }
    ...
    context 'with invalid task_id' do
      let(:params) { { task_id: 0, project_id: project.id, body: FFaker::Book.title } }
    ...
    context 'with valid image' do
      ...
      let(:params) { { task_id: task.id, project_id: project.id, body: FFaker::Lorem.paragraph, image: image } }
  ...
  describe 'DELETE destroy' do
    ...
    context 'with valid id' do
      let(:params) { { id: comment.id, task_id: task.id, project_id: project.id } }

      it { expect { delete :destroy, params: params }.to change(Comment, :count).by(-1) }
    end

    context 'with invalid id' do
      let(:params) { { id: 0, task_id: task.id, project_id: project.id } }
